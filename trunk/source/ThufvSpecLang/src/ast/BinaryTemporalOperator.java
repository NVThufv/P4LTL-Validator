...
//			return and_lhs_rhs.toString();
		...
		if (type == BinaryTemporalType.weakuntil) {
			// a W b == Ga || a U b
			AstNode globala = new UnaryTemporalOperator(UnaryTemporalType.always, 
					getOutgoingNodes().get(0));
			AstNode aUb = new BinaryTemporalOperator(BinaryTemporalType.until, 
					getOutgoingNodes().get(0), getOutgoingNodes().get(1));
			AstNode result = new BinaryTemporalOperator(BinaryTemporalType.or, 
					globala, aUb);
