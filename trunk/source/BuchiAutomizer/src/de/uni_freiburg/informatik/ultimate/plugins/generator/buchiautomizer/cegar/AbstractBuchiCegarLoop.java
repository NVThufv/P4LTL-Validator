...
//				int loopTime = 1;
//				int k = 10;
//				while (loopTime < k && lassoCheck.getLassoCheckResult().getContinueDirective() == ContinueDirective.REPORT_UNKNOWN) {
//					// if result was unknown, then try again but this time add one
//					// iteration of the loop to the stem.
//					// This allows us to verify Vincent's coolant examples
//					final TaskIdentifier unwindingTaskIdentifier =
//							new SubtaskAdditionalLoopUnwinding(taskIdentifier, loopTime);
//					mLogger.info("Result of lasso check was UNKNOWN. I will concatenate " +
//								 loopTime + " loop(s) to stem and try again.");
//					for(int loopUnwindingTime = 0; loopUnwindingTime < loopTime; loopUnwindingTime++)
//					{
//						final NestedRun<L, IPredicate> newStem =
//								mCounterexample.getStem().concatenate(mCounterexample.getLoop());
//						mCounterexample = new NestedLassoRun<>(newStem, mCounterexample.getLoop());
...
//				}
			...
			try {
				switch (cd) {
				case REFINE_BOTH:
					mAbstraction = refineFinite(refineBuchiAndReportRankingFunction(lassoCheck), lassoCheck);
					break;
				case REFINE_FINITE:
