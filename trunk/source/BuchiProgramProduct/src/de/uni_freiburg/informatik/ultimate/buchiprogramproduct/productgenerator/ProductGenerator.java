...
public final class ProductGenerator {
	...
	public ProductGenerator(final INestedWordAutomaton<CodeBlock, String> nwa, final BoogieIcfgContainer rcfg,
		...
		// add transaction step info
		annotateTransactionInfo();
	...
	/**
	 * add transaction info -- annotate the corresponding step with LTLStepAnnotation.
	 */
	private void annotateTransactionInfo()
	{
		mLogger.info("----- Annotating TransactionInfo ...");
		final Set<Entry<String, Map<DebugIdentifier, BoogieIcfgLocation>>> programPoints = mRcfgRoot
		...
		for (final Entry<String, Map<DebugIdentifier, BoogieIcfgLocation>> pairs : programPoints) {
			for (final Entry<DebugIdentifier, BoogieIcfgLocation> loc : pairs.getValue().entrySet()) {
				for (final IcfgEdge rcfgEdge : loc.getValue().getOutgoingEdges()) {
					if (!(rcfgEdge instanceof Return)) {
						// skip all edges that are not return or call edges
						continue;
					}
					Call correspondingCall = null;
					if(rcfgEdge instanceof Call)
						correspondingCall = (Call) rcfgEdge;
					else if(rcfgEdge instanceof Return)
						correspondingCall = ((Return) rcfgEdge).getCorrespondingCall();
						
					assert correspondingCall != null : "Corresponding Call is null: " + rcfgEdge;
					
					String calledMethod = correspondingCall.getCallStatement().getMethodName();
					mLogger.info("+++++ Call method name: " + calledMethod);
					if(calledMethod == "main")
					{
						mLogger.info("----- Handling transaction edge from " + rcfgEdge.getSource() + " to " + rcfgEdge.getTarget());
						new LTLStepAnnotation().annotate(rcfgEdge);
					}
				}
			}
		}
...
//				mLogger.info("==== Adding self loop for point: " + currentPoint + " New edge: " + newEdge);
			}
...
//				if (mNWA.isFinal(nwaState)) {
	...
	private ArrayList<IcfgLocation> getCFGFinalStates() {
		...
		for(final BoogieIcfgLocation loc : mRCFGLocations) {
			mLogger.info("==== location: " + loc.toString());
		...
		for (final BoogieIcfgLocation origRcfgSourceLoc : mRCFGLocations) {
			for (final IcfgEdge rcfgEdge : origRcfgSourceLoc.getOutgoingEdges()) {
				...
				if (isNonProductNode(origRcfgSourceLoc) && isNonProductNode(origRcfgTargetLoc)) {
					mLogger.info("---------- Handling double non product return edge from " + rcfgEdge.getSource() + " to " + rcfgEdge.getTarget());
					createReturnEdgesNonProduct(origRcfgSourceLoc, origRcfgTargetLoc, returnEdge);
				} else if (isNonProductNode(origRcfgSourceLoc)) {
					mLogger.info("---------- Handling double src product return edge from " + rcfgEdge.getSource() + " to " + rcfgEdge.getTarget());
					createReturnEdgesNonProductToProduct(origRcfgSourceLoc, origRcfgTargetLoc, returnEdge);
				} else {
					mLogger.info("---------- Handling double product return edge from " + rcfgEdge.getSource() + " to " + rcfgEdge.getTarget());
...
//					createReturnEdgesOther(origRcfgSourceLoc, returnEdge, false);
					// Thufv: why UA use false?
					createReturnEdgesOther(origRcfgSourceLoc, returnEdge, isProgramStep);
				}
			}
		}
	}
	...
	private void createEdgesProduct(final BoogieIcfgLocation origRcfgSourceLoc, final IcfgEdge rcfgEdge) {
		...
		if(isProgramStep)
		{
			mLogger.info("++++ Encountering program step " + rcfgEdge);
		}
		...
		for (final String nwaLoc : mNWA.getStates()) {
			...
			if (rcfgEdge instanceof StatementSequence) {
				...
				mLogger.info("Handling product edge call: " + (Call) rcfgEdge);
				handleEdgeCall(productSourceLoc, nwaLoc, (Call) rcfgEdge, origRcfgSourceLoc, isProgramStep);
			} else if (rcfgEdge instanceof Summary) {
				mLogger.info("Handling product edge summary: " + (Summary) rcfgEdge);
				handleEdgeSummary(productSourceLoc, nwaLoc, (Summary) rcfgEdge, isProgramStep);
			} else {
...
//		}
		...
		// Thufv: changing implementation, for autTrans.getSucc() may not contain nwaLoc
		if(isProgramStep)
		{
			mLogger.info("==== Handling return program step: " + returnEdge);
			for (final OutgoingInternalTransition<CodeBlock, String> autTrans : mNWA.internalSuccessors(nwaLoc)) {
				BoogieIcfgLocation targetpp = mProductLocations
						.get(ProductLocationNameGenerator.generateStateName(origRcfgTargetLoc, autTrans.getSucc()));
				if (targetpp == null) {
		...
		else
		{
			...
			AssumeStatement stmtTrue = generateNeverClaimAssumeStatement(new BooleanLiteral(null, true));
			StatementSequence newSS = mCodeblockFactory.constructStatementSequence(helper, targetpp, stmtTrue, Origin.IMPLEMENTATION);
			mapNewEdge2OldEdge(newSS, null);
		}
	}

	private void handleEdgeSummary(final BoogieIcfgLocation productSourceLoc, final String nwaLoc,
			final Summary summary, boolean isProgramStep) {
		...
		// Thufv: adding operation for program step
		if(isProgramStep)
		{
			for (final OutgoingInternalTransition<CodeBlock, String> autTrans : mNWA.internalSuccessors(nwaLoc)) {
				targetpp = mProductLocations.get(ProductLocationNameGenerator
		...
		{
			targetpp = mProductLocations.get(ProductLocationNameGenerator
					.generateStateName((BoogieIcfgLocation) summary.getTarget(), nwaLoc));
			
			final List<CodeBlock> sumAndSs = new ArrayList<>();
			AssumeStatement stmtTrue = generateNeverClaimAssumeStatement(new BooleanLiteral(null, true));
			final StatementSequence seq = mCodeblockFactory.constructStatementSequence(productSourceLoc, targetpp, stmtTrue, Origin.IMPLEMENTATION);
			tfb.addTransFormula(seq, ((BoogieIcfgLocation) summary.getSource()).getProcedure());
...
//		}
		...
		// Thufv: changing impl for this state may not be the suc state
		if(isProgramStep)
		{
			for (final OutgoingInternalTransition<CodeBlock, String> autTrans : mNWA.internalSuccessors(nwaSourceState)) {
				final BoogieIcfgLocation targetpp = mProductLocations
		...
		{
			BoogieIcfgLocation targetpp = mProductLocations
					.get(ProductLocationNameGenerator.generateStateName(origRcfgTargetLoc, nwaSourceState));
			AssumeStatement stmtTrue = generateNeverClaimAssumeStatement(new BooleanLiteral(null, true));
			StatementSequence newSS = mCodeblockFactory.constructStatementSequence(helper, targetpp, stmtTrue, Origin.IMPLEMENTATION);
			mapNewEdge2OldEdge(newSS, null);
		}
