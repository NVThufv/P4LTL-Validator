...
cp ${EXESCRIPT} "$TARGETDIR"/
cp ${CHECKSCRIPT} "$TARGETDIR"/
cp ${TOOLCHAIN} "$CONFIGDIR"/
cp ${SETTINGS} "$CONFIGDIR"/
...
sed "s/toolname =.*/toolname = \'$TOOLNAME\'/g" "$TARGETDIR"/Ultimate.py > "$TARGETDIR"/Ultimate.py.tmp && mv "$TARGETDIR"/Ultimate.py.tmp "$TARGETDIR"/Ultimate.py && chmod a+x "$TARGETDIR"/Ultimate.py

## creating new zipfile 
echo "Creating .zip"
zip -q ${ZIPFILE} -r "$TARGETDIR"/*
